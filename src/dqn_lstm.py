...
class DQN_LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # extrai último timestep para decisão de ação
...
class TradingEnvironmentLSTM:
    def __init__(self, data: pd.DataFrame, window_size: int = 21, initial_balance: float = 10000.0):
        self.data = data
        self.window_size = window_size  # contexto histórico para lstm
    ...
    def _get_observation(self):
        ...
        if end_idx > len(self.data):
            # retorna sequência vazia quando fora dos limites
            sequence = np.zeros((self.window_size, 15))
            return sequence
        
        window_data = self.data.iloc[start_idx:end_idx]
        
        # preenche com primeira observação se histórico insuficiente
    ...
    def _normalize_sequence(self, sequence):
        ...
        # transformação log para lidar com valores extremos de volume
        vol_data = sequence[:, 8]
        if vol_data.max() > 0:
...
def train_dqn_lstm(env, episodes=5000, batch_size=32, buffer_size=100000,
    ...
    for episode in tqdm(range(episodes), desc="Training Episodes"):
        ...
        while True:
            ...
            if random.random() < epsilon:
                ...
                with torch.no_grad():
                    # adiciona dimensão de batch para lstm
            ...
            current_rsi = state[-1, 6] * 100
            current_pnl = (state[-1, 12] - 0.5) * 100
            cash_pct = state[-1, 13] * 200
            position_pct = state[-1, 14] * 200
        ...
        while step < len(val_data) - 21:
            ...
            if current_idx < len(val_data):
                output_data.append({
                    ...
                    'state': state.flatten().tolist(),  # achata sequência para armazenamento
                    'action': action,
